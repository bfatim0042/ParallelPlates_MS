...
int main(void) {
    // Parameters
    int n_layers = 2;   // Number of layers in z direction 
    int Nx = 10;        // Number of particles along x
    int Ny = 10;        // Number of particles along y
    const double diameter = 1.0;
    double H = 2.0;    // Target box height
    double eta = 0.55; // Target packing fraction

    // Triangular lattice spacing
    //double ax = diameter * sqrt(M_PI / (2.0 * sqrt(3.0) * eta)) + 1e-10; // spacing along x, small gap to avoid overlaps
    //double ay = ax * sqrt(3) / 2.0; // spacing along y (triangular lattice)
    ...
    double ax = sqrt(A_needed / (Nx * Ny * sqrt(3.0) / 2.0)) + 1e-10;
    if (ax < diameter) {
        ax = diameter + 1e-10;
    }
    double ay = ax * sqrt(3.0) / 2.0;
    ...
    for (int layer = 0; layer < n_layers; layer++) {
        ...
        // Alternate layer offset (ax/2, ay/2)
        double x_offset = (layer % 2) * 0.5 * ax; // offset triggers every other layer
        double y_offset = (layer % 2) * 0.5 * ay;

        for (int j = 0; j < Ny; j++) {
            double row_shift = (j % 2) * 0.5 * ax; // Shift every other row for triangular lattice
            for (int i = 0; i < Nx; i++) {
    ...
    //Write to file
    char filename[256];
    sprintf(filename, "ovito/trilat_H%.1f_eta%.2f.dat", H, eta);
